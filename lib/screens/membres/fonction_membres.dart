...
class FonctionMembres extends StatefulWidget {
  int? fonction_id;
  User? user;
  FonctionMembres({required this.fonction_id, required this.user});

  @override
  State<FonctionMembres> createState() => _FonctionMembresState();
}

class _FonctionMembresState extends State<FonctionMembres> {
  int? fonction_id;
  User? user;
  int totalMembre = 0;
  String? recherche;
  ...
  Future _getallFonctionMembres() async{
    ApiResponse apiResponse = await filtreMembresParFonction(fonction_id);
    setState(() {
      search.clear();
    });
  ...
  Future _searchMembres(String? search) async{
    ApiResponse apiResponse = await searchMembresFonction(search, fonction_id);
    loading = false;
    if(apiResponse.error == null){
      List<dynamic> membresList = apiResponse.data as List<dynamic>;
      List<Membres> membres = membresList.map((p) => Membres.fromJson(p)).toList();
  ...
  void initState() {
    user = widget.user;
    fonction_id = widget.fonction_id;
    _getallFonctionMembres();
    super.initState();
  }
